...
Options: `-Pjmh.include`, `-Pjmh.iterations`, `-Pjmh.warmupIterations`, `-Pjmh.fork`. Results also land in
...
`Vm` builds the machine and `R5Assembler` hand-assembles instruction streams, so benchmarks need no cross toolchain.
`Vm.bare(size)` gives a machine in machine mode with translation off, which isolates dispatch; `Vm.paged(size)` maps RAM
through a three level Sv39 hierarchy of 4KiB pages and
