...
public abstract class AbstractVirtIODevice implements MemoryMappedDevice, InterruptSource, Resettable {
    ...
    private final class QueueContext implements VirtqueueContext {
        @Override
        public void error() {
        ...
        public long getNegotiatedFeatures() {
            return AbstractVirtIODevice.this.getNegotiatedFeatures();
        }

        @Override
        public void raiseUsedBufferInterrupt() {
            interruptStatus |= VIRTIO_IRQ_USED_BUFFER_MASK;
            updateInterrupts();
        }

        @Override
        public void dispatchQueueNotification(final int queueIndex) throws VirtIODeviceException, MemoryAccessException {
            handleQueueNotification(queueIndex);
        }
    }
}
